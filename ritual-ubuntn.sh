...
for dep in "${ubuntu_deps[@]}"; do
    ...
    while true; do
        if sudo apt-get install -y "$dep"; then
            ...
            warn "⚠️ $dep 安装失败，第 $attempt 次重试（10秒后）..."
            sleep 10
            ((attempt++))
        fi
    done
    ...
    if [ ! -f "/etc/apt/keyrings/docker.gpg" ]; then
        ...
        while true; do
            if curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo gpg --dearmor -o /etc/apt/keyrings/docker.gpg; then
                ...
                info "✅ Docker GPG 密钥添加成功"
                break
            else
                warn "Docker GPG 密钥添加失败，第 $attempt 次重试（10秒后）..."
                sleep 10
                ((attempt++))
            fi
        done
    ...
    if [ ! -f "/etc/apt/sources.list.d/docker.list" ]; then
        ...
        while true; do
            if sudo apt-get update -y; then
                ...
                warn "Docker 仓库更新失败，第 $attempt 次重试（10秒后）..."
                sleep 10
                ((attempt++))
            fi
        done
    ...
    for pkg in "${docker_pkgs[@]}"; do
        ...
        while true; do
            if sudo apt-get install -y "$pkg"; then
                ...
                warn "⚠️ $pkg 安装失败，第 $attempt 次重试（10秒后）..."
                sleep 10
                ((attempt++))
            fi
        done
...
if pidof systemd &>/dev/null; then
    ...
    while true; do
        ...
        if sudo systemctl start docker; then
            ...
            warn "Docker 服务启动失败，第 $attempt 次重试（10秒后）..."
...
if [ "$skip_to_deploy" = "true" ] || ([ "$yn" != "退出" ] && [ "$update_config_and_restart" != "true" ]); then
    ...
    while true; do
        ...
        if [ -n "$chain_id" ]; then
            ...
            warn "RPC 连接失败，第 $attempt 次重试（10秒后）..."
            sleep 10
            ((attempt++))
        fi
    done
        ...
        while true; do
            if curl -L https://foundry.paradigm.xyz | bash; then
                ...
                if foundryup; then
                    ...
                    warn "Foundry 更新失败，第 $attempt 次重试（10秒后）..."
                fi
            else
                warn "Foundry 安装失败，第 $attempt 次重试（10秒后）..."
            fi
            sleep 10
            ((attempt++))
        done
...
if [ ! -d "lib" ]; then
    ...
    attempt=1
    while true; do
        if forge install foundry-rs/forge-std OpenZeppelin/openzeppelin-contracts; then
            info "✅ 合约依赖安装成功"
            break
        else
            warn "合约依赖安装失败，第 $attempt 次重试（10秒后）..."
            sleep 10
            ((attempt++))
        fi
    done
...
while true; do
    if forge script script/Deploy.s.sol:Deploy --rpc-url "$RPC_URL" --private-key "$PRIVATE_KEY" --broadcast --verify; then
        ...
        warn "合约部署失败，第 $attempt 次重试（10秒后）..."
        sleep 10
        ((attempt++))
    fi
done
